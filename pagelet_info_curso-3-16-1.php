...
class pagelet_info_curso extends pagelet
{
    ...
    {
        return ($this->controlador->hay_var('mensajes'))? $this->controlador->get_var('mensajes') : array();
    }

    function get_curso()
    {
        return $this->controlador->get_var('curso');
    }

    function get_comision()
    {
        return $this->controlador->get_var('comision');
    }

    function get_curso_elemento(){
        $curso = $this->controlador->get_var('curso');
        return $curso['elemento'];
    }

    function get_curso_nombre()
    {
        $info_curso = $this->modelo()->info__curso($this->get_curso_elemento());
        return $info_curso['nombre'];
    }

    function get_curso_codigo()
    {
        $info_curso = $this->modelo()->info__curso($this->get_curso_elemento());
        return $info_curso['codigo'];
    }

    function get_curso_ra()
    {
        $info_curso = $this->modelo()->info__curso($this->get_curso_elemento());
        return implode(', ', explode('|', $info_curso['responsables_academicas']));
    }

    protected function get_max_inscripcion_por_prioridad()
    {
        $info_curso = $this->modelo()->info__curso($this->get_curso_elemento());
        $max = ($info_curso['insc_cursada_por_prioridad'] == \comunes::si) ? $info_curso['prioridad_cantidad_maxima'] : 1;
        return $max;
    }

    protected function add_estados_pagelet(){
        $this->data['estado_inicial'] = self::INICIAL;
        $this->data['estado_curso_seleccionado'] = self::CURSO_SELECCIONADO;
        $this->data['estado_error_curso_seleccionado'] = self::ERROR_CURSO_SELECCIONADO;
        $this->data['estado_reinscripcion'] = self::REINSCRIPCION;
    }
    ...
    {
        //$this->data = array();
        $this->add_mensajes_js();
        $this->add_estados_pagelet();
        $this->add_datos_generales_pagelet();

        switch ($this->estado) {
            case self::INICIAL:
                $this->prepare_inicial();
                break;
            // seleccion� un curso que no estoy inscripto
            case self::CURSO_SELECCIONADO:
                $this->prepare_curso_seleccionado();
                break;

            // seleccion� un curso que no estoy inscripto y hubo error de controles
            case self::ERROR_CURSO_SELECCIONADO:
                //$this->prepare_error_materia_seleccionada();
                break;

            // estoy inscripto a un curso pero quiero reinscribirme en otra comisi�n
            case self::REINSCRIPCION:
                //$this->prepare_reinscripcion();
                break;
        }
    }
    ...
    protected function prepare_inicial()
    {
        ...
        $inscripciones = guarani::persona()->cursadas()->get_inscripciones($propuesta_seleccionada);
    ...
    protected function esta_inscripto_a_comision($comisiones_inscriptas, $id_comision)
    {
        foreach ($comisiones_inscriptas as $com_insc) {
            if ($id_comision == $com_insc['comision']) {
                return true;
            }
        }
    ...
    protected function get_comision_inscripta_info($comisiones_inscriptas, $id_comision)
    {
        foreach ($comisiones_inscriptas as $com_insc) {
            if ($id_comision == $com_insc['comision']) {
                return $com_insc;
        ...
        throw new error_guarani("Se est� buscando la comision $id_comision en la lista de comisiones inscriptas y no existe");
    }

    protected function get_comision_ignorar()
    {
        if ($this->controlador->hay_var('ignorar_orden_comision')) {
            return $this->controlador->get_var('ignorar_orden_comision');
        }

        return false;
    }

    protected function get_vinculo_comp($inscripcion, $actividad = null)
    {
        $parametros = array(
        ...
        if (isset($actividad)){
            $parametros['curso'] = $actividad;
        }

        return kernel::vinculador()->crear(
            kernel::ruteador()->get_id_operacion(),
            'generar_comprobante',
            $parametros
        );
    }

    protected function get_vinculo_imp_comp($inscripcion, $actividad = null)
    {
        $parametros = array(
            'inscripcion' => $inscripcion,
            'i' => 1
    ...
    protected function get_vinculo_mail_comp($inscripcion, $actividad = null)
    {
        $parametros = array('inscripcion' => $inscripcion);
    ...
    protected function agregar_comision_inscripta($orden, $inscripcion_a_comision, $actividad, $comision, $agregar_al_principio)
    {
        $inscripcion_a_comision['inscripto'] = true;
        $inscripcion_a_comision['id'] = $comision[catalogo::id];
        $inscripcion_a_comision['actividad'] = $actividad;
        $inscripcion_a_comision['url_comp'] = $this->get_vinculo_comp($inscripcion_a_comision[catalogo::id], $actividad);
        $inscripcion_a_comision['url_imp_comp'] = $this->get_vinculo_imp_comp($inscripcion_a_comision[catalogo::id], $actividad);
        $inscripcion_a_comision['url_mail_comp'] = $this->get_vinculo_mail_comp($inscripcion_a_comision[catalogo::id], $actividad);
        $inscripcion_a_comision['url_baja'] = kernel::vinculador()->crear(kernel::ruteador()->get_id_operacion(), 'baja');
        $inscripcion_a_comision['cobrable'] = $comision['cobrable'];
        if(isset($comision['costo'])) $inscripcion_a_comision['costo'] = $comision['costo'];
        if(isset($comision['label_costo'])) $inscripcion_a_comision['label_costo'] = $comision['label_costo'];

        $inscripcion_a_comision['pago_pendiente'] = $inscripcion_a_comision['pago_pendiente'];
        $inscripcion_a_comision['estado_inscripcion'] = $inscripcion_a_comision['estado'];

        if (!$agregar_al_principio) {
            $this->data['comisiones'][$orden] = $inscripcion_a_comision;
        } else {
            unset($this->data['comisiones'][$orden]);
            $this->data['comisiones'] = array($orden => $inscripcion_a_comision) + $this->data['comisiones'];
        }
    }

    protected function agregar_comision_no_inscripta($orden, $actividad, $comision)
    {
        $this->data['comisiones'][$orden]['inscripto'] = false;
        $this->data['comisiones'][$orden]['id'] = $comision[catalogo::id];
        $this->data['comisiones'][$orden]['actividad'] = $actividad;
        $this->data['comisiones'][$orden]['url_inscribirse'] = kernel::vinculador()->crear(kernel::ruteador()->get_id_operacion(), 'inscribir');
        $this->data['comisiones'][$orden]['tiene_cupo'] = ($comision['tiene_cupo'] == \comunes::si) ? true : false;
        if (isset($comision['cupo']) && $comision['cupo'] > 0 ) {
            $this->data['comisiones'][$orden]['cupo_definido'] = true;
            $porcentaje = round (($comision['cant_inscriptos'] <= 0 ? 0 :$comision['cant_inscriptos']) / $comision['cupo'] * 100,0);
            $this->data['comisiones'][$orden]['porcentaje_cupo'] = $porcentaje > 100 ? 100 : $porcentaje;
        } else {
            $this->data['comisiones'][$orden]['mostrar_barra_cupo'] = false;
        }

        $this->data['comisiones'][$orden]['inscripcion_cerrada'] = ($comision['inscripcion_cerrada'] == \comunes::si) ? true : false;
    }
    ...
    protected function prepare_error_curso_seleccionado()
    {
        $this->data['curso_nombre'] = $this->get_curso_nombre();
        $this->data['curso_codigo']	= $this->get_curso_codigo();
        $this->data['mensajes'] = $this->get_mensajes();
