...
class controlador extends controlador_g3w2
{
    ...
    function accion__generar_comprobante()
    {
        ...
        // Verifico si existe la inscripci�n, puede ser que se haya dado de baja desde Gesti�n
        ...
        // Si no existe la inscripci�n
        if(!$existe_inscripcion){
            // Limpio la cache de inscripcion a cursadas
            guarani::persona()->cursadas()->reset();
            // Lanzo una excepci�n para generar un error
    ...
    function accion__baja()
    {
        ...
        try {
            ...
            //valido el modo de devoluci�n del pago
    ...
    protected function validar_modo_devolucion_pago($modo_devolucion){
        ...
        //if (!in_array($modo_devolucion, array(\cobro::SUSCRIPCIONES_NOVEDADES_MODO_DEVOLUCION_CREDITO, \cobro::SUSCRIPCIONES_NOVEDADES_MODO_DEVOLUCION_AGENTE_COBRANZA))) {
            $e = new error_guarani('');
    ...
    function accion__existe_inscripcion() {
        ...
        try {
            ...
            // Si no existe la inscripci�n limpio la cache de inscripcion a cursadas
    ...
    function accion__es_comision_cobrable() {
        ...
        try {
            ...
            $hubo_error = true;
            $msj_error = kernel::traductor()->trans('inscripcion_cursos.error_averiguar_comisi�n_cobrable');
            kernel::log()->add_error($e);
            kernel::renderer()->add_to_ajax_response("hubo_error", $hubo_error);
            kernel::renderer()->add_to_ajax_response("msj_error", $msj_error);
        }
    ...
    function accion__verificar_sol_cons_ext(){
        ...
        if(kernel::request()->isPost()){
            try{
                ...
                if($token_sq != $nuevo_token_sq){//Si el token expir� lo reemplazo
